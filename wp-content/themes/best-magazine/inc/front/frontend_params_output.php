 ...
 */
  ...
  */
  ...
  public function color_control(){
    ...
    $text_headers_color = esc_html( $this->get_param('[colors_active][colors][text_headers_color][value]', array(), "#000000" ));
...
public function top_posts(){
  global $post;
  $show_top_posts = $this->get_param('hide_top_posts');
  if (!$show_top_posts) {
  return;
  }
  $top_post_categories = implode(',',$this->get_param('top_post_categories', array(), array('')));
  $top_post_cat_name = $this->get_param('top_post_cat_name');
  
  $order = $this->get_param('top_post_order', array(), array('date'));
  
  $order = $order[0];
  $orderby = $this->get_param('top_post_orderby', array(), array('desc'));
  $orderby = $orderby[0];
  
  $grab_image = $this->get_param('grab_image');
  $lbox_width = $this->get_param('lbox_image_width');
  $lbox_height = $this->get_param('lbox_image_height');
  $lbox_enable = $this->get_param('lbox_enable', array(), '1');
  if($lbox_enable == '1'){
    $lbox_enable = true;
  }
  else{
                ...
                if($wp_query->have_posts()) {
                  while ($wp_query->have_posts()) {
                    ...
                    if( $thumb_url ){
                      ...
                      $thumb_url = Best_magazine_frontend_functions::catch_that_image();
                ...
                <li class="top_effect">
                    ...
                    <div class="image-block <?php echo $thumb_div_class; ?>">
                      ...
                      <?php 
                        if($grab_image)
                         {
                        echo Best_magazine_frontend_functions::display_thumbnail(340,200); 
                         }
                         else 
                         {
                        echo Best_magazine_frontend_functions::thumbnail(340,200);
                         }  
                    ...
                    <h3><a href="<?php the_permalink(); ?>"  rel="image-block-<?php echo $id; ?>-title"><?php the_title(); ?></a></h3>
                  <div class="text">
                    <p><?php Best_magazine_frontend_functions::the_excerpt_max_charlength(120); ?></p>
...
global $post;
$hide_category_tabs_posts = $this->get_param('hide_category_tabs_posts');
$home_page_tabs_exclusive = $this->get_param('home_page_tabs_exclusive');
$grab_image = $this->get_param('grab_image');
           ...
           if ( have_posts() ) : while ( have_posts() ) : the_post();
              ...
              <div class="thumbnail-block <?php echo $thumb_div_class; ?>">
                  <a class="image-block" href="<?php echo get_permalink(); ?>"><?php
                    if($grab_image)
                     {
                    echo Best_magazine_frontend_functions::display_thumbnail(150,150); 
                     }
                     else 
                     {
                    echo Best_magazine_frontend_functions::thumbnail(150,150);
                     } ?>
              </a>
                </div>
              <div class="text">
...
public function home_video_post(){
$home_video_post = $this->get_param('home_video_post');
$hide_video_post = $this->get_param('hide_video_post');
$video_post_name = $this->get_param('video_post_name');

$home_video_post=isset($home_video_post[0]) ? get_post($home_video_post[0]) : null; 

if($home_video_post==NULL){
$home_video_post=get_posts();
/*the most recent post*/
$home_video_post=empty($home_video_post) ? null : $home_video_post[0];
}

if ($hide_video_post && $home_video_post){ ?>
<div id="videos-block" class="content-inner-block">
  ...
  <div class="full-width">
    <?php echo get_the_post_thumbnail( $home_video_post->ID,array(260,220)); ?>
          <h3><?php echo $home_video_post->post_title; ?></h3>
...
public function content_posts() {
  ...
  $hide_content_posts = $this->get_param('hide_content_posts');
  ...
  $grab_image= $this->get_param('grab_image');
  $show_thumbnail = $this->get_param('show_thumbnails');

  $content_post_categories = $this->get_param('content_post_categories', array(), array(''));
  $content_post_cat_name = $this->get_param('content_post_cat_name');
  $blog_style = $this->get_param('blog_style');
  $content_post_categories = implode(',',$content_post_categories);

  $paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
  $order_content = $this->get_param('content_post_order', array(), array('date'));
  $order_content = $order_content[0];
  $orderby_content = $this->get_param('content_post_orderby', array(), array('desc'));
      ...
      <?php 
        ...
        ?>
          ...
          <?php if($show_thumbnail): ?>
            <?php if(has_post_thumbnail() || (Best_magazine_frontend_functions::post_image_url() && $blog_style && $grab_image)){
              ?>
                <div class="img_container unfixed">
                  <?php echo Best_magazine_frontend_functions::auto_thumbnail($grab_image); ?>
                </div>
                <div class="clear"></div>
              <?php
             } ?>
          ...
          {
             Best_magazine_frontend_functions::the_excerpt_max_charlength(250);
          }
          else 
          {
             the_content(); 
...
function content_posts_for_home() {
global $wp_query,$paged,$wdwt_front;

$hide_content_posts = $this->get_param('hide_content_posts');
$hide_video_post = $this->get_param('hide_video_post'); 
$date_enable = $this->get_param('date_enable'); 
$blog_style = $this->get_param('blog_style'); 
$grab_image = $this->get_param('grab_image'); 
if(is_home()){  
    ...
    <div class="blog-post home-post">        
      <a class="title_href" href="<?php echo get_permalink() ?>">
         ...
         <div class="home-post-date">
          <?php echo Best_magazine_frontend_functions::posted_on();?>
         </div>
        <?php } 
         if($grab_image)
         {
